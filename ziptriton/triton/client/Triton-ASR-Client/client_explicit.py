...
def load_manifests(dir_path):
    ...
    transcripts_path = dir_path / "text"
...
async def send_streaming(
    ...
    for i, dp in enumerate(dps):
        ...
        for idx, seg in enumerate(wav_segs):
            ...
            chunk_samples = int(chunk_in_secs * sample_rate)
    ...
    return total_duration, results, latency_data, end_point_data
...
async def main():
    ...
    # Load models
    if not await triton_client.is_model_ready("decoder"):
        await triton_client.load_model("decoder")
    if not await triton_client.is_model_ready("joiner"):
        await triton_client.load_model("joiner")
    # if not await triton_client.is_model_ready("adapter"):
    #     await triton_client.load_model("adapter")


    if args.domain == "base":
        if not await triton_client.is_model_ready("transducer"):
            await triton_client.load_model("transducer")
            print(f"Model transducer is ready")
    elif args.domain == "vnpost":
        if not await triton_client.is_model_ready("transducervnpost"):
            await triton_client.load_model("transducervnpost")
            print(f"Model transducervnpost is ready")
    elif args.domain == "g7":
        if not await triton_client.is_model_ready("transducerg7"):
            # await load_model(triton_client, "transducerg7")
            await triton_client.load_model("transducerg7")
            print(f"Model transducerg7 is ready")

    

    #Model metadata
    # metadata = await triton_client.get_model_metadata(f"transducer{args.domain}")
    metadata = await triton_client.get_model_metadata("decoder")
    print(metadata)
        ...
        if args.streaming:
            ...
            task = asyncio.create_task(
                send_streaming(
                    ...
                    log_interval=args.log_interval,
                    compute_cer=args.compute_cer,
                    model_name=f"{args.model_name}{args.domain}",
                    chunk_in_secs=args.chunk_size
                    * args.subsampling
                    * frame_shift_ms
                    / 1000,
    ...
    if args.streaming or args.simulate_streaming:
        ...
        s += f"latency_95_percentile: {np.percentile(latency_list, 95) * 1000.0:.2f}\n"
        s += f"latency_99_percentile: {np.percentile(latency_list, 99) * 1000.0:.2f}\n"
        s += f"average_latency_ms: {latency_ms:.2f}\n"

        rtf_list = [chunk_end/chunk_duration for (chunk_end, chunk_duration) in latency_data]
        rtf = sum(rtf_list) / float(len(rtf_list))
        rtf_variance = np.var(rtf_list, dtype=np.float64)
        s += f'rtf: {rtf:.2f}\n'
        s += f"rtf_variance: {rtf_variance:.2f}\n"
    ...
    with open("log/rtf.txt", "w") as f:
        f.write(s)

    name = Path(args.manifest_dir).stem.split(".")[0]
    results = sorted(results)
    store_transcripts(filename=f"log/recogs-{name}.txt", texts=results)
    with open(f"log/errs-{name}.txt", "w") as f:
        write_error_stats(f, "log/test-set", results, enable_log=True)

    with open(f"log/errs-{name}.txt", "r") as f:
    ...
    await triton_client.unload_model("decoder")
    await triton_client.unload_model("joiner")
    await triton_client.unload_model(f"{args.model_name}{args.domain}")
