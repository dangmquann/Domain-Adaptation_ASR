...
def modified_beam_search_lm(
    ...
    am_scores = torch.tensor([hyp.log_prob.item() for hyps in B for hyp in hyps])
    ...
    for seq in candidate_seqs:
        ...
        LODR_score.append(ngram_lm.score(tokens))

    LODR_score = torch.tensor(LODR_score).to(device) * math.log(10)
    # print('LODR score: ', LODR_score)
    # print('am score: ', am_scores.values)
    for lm_scale in ngram_lm_scale:
        for lodr_scale in LODR_scale_list:
            tot_score = am_scores.values / lm_scale + LODR_score * lodr_scale
            key = f"nnlm_scale_{lm_scale:.2f}_lodr_scale_{lodr_scale:.2f}"
            ragged_tot_scores = k2.RaggedTensor(shape=am_scores.shape, value=tot_score)
            max_indexes = ragged_tot_scores.argmax().tolist()
            unsorted_hyps = [candidate_seqs[idx] for idx in max_indexes]
            for idx in range(batch_size):
                streams[idx].best_hyp[key] = unsorted_hyps[idx]
