...
from streaming_beam_search import (
    ...
    modified_beam_search_lm
)
from torch import Tensor, nn
from torch.nn.utils.rnn import pad_sequence
from train_2 import add_model_arguments, get_params, get_model
...
def get_parser():
    ...
    parser.add_argument(
        "--lang-dir",
        type=Path,
        default='/home/quangnv53/Project_2/icefall/egs/librispeech/ASR/data/lm',
        help="The number of streams that can be decoded parallel.",
    )
...
def decode_one_chunk(
    ...
    elif params.decoding_method == "modified_beam_search_lm":
        modified_beam_search_lm(
            model=model,
            streams=decode_streams,
            encoder_out=encoder_out,
            ...
            LODR_scale_list = LODR_scale_list,
            num_active_paths=params.num_active_paths,
        )
    else:
...
def decode_dataset(
    ...
    ngram_lm = None,
    ngram_lm_scale: List[float] = None,
    LODR_scale_list: List[float] = None,
    ...
    log_interval = 100
    if params.decoding_method == "modified_beam_search_lm":
        decode_results = {}
        for lm_scale in ngram_lm_scale:
    ...
    for num, cut in enumerate(cuts):
        ...
        samples = torch.cat([torch.zeros(240,dtype=torch.float32), samples], dim=0)
        fbank = Fbank(opts)
        ...
        while len(decode_streams) >= params.num_decode_streams:
            ...
            if params.decoding_method == "modified_beam_search_lm":
                for i in sorted(finished_streams, reverse=True):
                    for key, val in decode_streams[i].best_hyp.items():
                        decode_results[key]+= [
                        (
                            decode_streams[i].id,
    ...
    if params.decoding_method == "modified_beam_search_lm":
        ...
        if params.decoding_method == "greedy_search":
            ...
            key = f"num_active_paths_{params.num_active_paths}_ngram_lm_scale_{ngram_lm_scale}"
        else:
            raise ValueError(f"Unsupported decoding method: {params.decoding_method}")
...
def main():
    ...
    if params.decoding_method == "modified_beam_search_lm":
        ...
        lm_scale_list = [1]
        LODR_scale_list = [-0.001, -0.01, -0.1]
    else:
        ngram_lm = None
        ngram_lm_scale = None
