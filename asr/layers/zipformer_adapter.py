...
class Zipformer2Encoder(nn.Module):
    ...
    def __init__(
        ...
        if use_last_adapter:
            self.last_adapter = AdapterModule(
    ...
    def forward(
        ...
        if self.last_adapter is not None:
            output = self.last_adapter(output)
        return output
...
class Zipformer2NoAdapter(nn.Module):
    def __init__(self, **kwargs):
        # Force off the adapter
        kwargs["use_adapters"] = False
        super().__init__()
        self.encoder = Zipformer2(**kwargs)
    
    def forward(self, x: torch.Tensor, x_lens: torch.Tensor, src_key_padding_mask: Optional[torch.Tensor] = None) -> Tuple[torch.Tensor, torch.Tensor]:
        return self.encoder(x, x_lens, src_key_padding_mask)
    
    def streaming_forward(self, x: torch.Tensor, x_lens: torch.Tensor, left_context_len: int, src_key_padding_mask: Optional[torch.Tensor] = None) -> Tuple[torch.Tensor, torch.Tensor]:
        return self.encoder.streaming_forward(x, x_lens, left_context_len, src_key_padding_mask)
