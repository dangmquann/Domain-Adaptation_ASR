...
class Zipformer2(EncoderInterface):
    ...
    def streaming_forward(
        ...
        # use loggging.info to print debug information
        logging.info(f"input tensor shape: {x.shape}")
        ...
        if torch.jit.is_scripting() or torch.jit.is_tracing():
            ...
            with warnings.catch_warnings():
                warnings.simplefilter("ignore")
                lengths = (x_lens + 1) // 2
        logging.info(f"output tensor shape: {x.shape}")
...
class Zipformer2EncoderLayer(nn.Module):
    ...
    def __init__(
        ...
        # ),
        #  attention_skip_rate: FloatLike = ScheduledFloat(
        #     (0.0, 0.2), (2000.0, 0.05), (8000, 0.0), default=0
        # ),
        # conv_skip_rate: FloatLike = ScheduledFloat(
        #     (0.0, 0.2), (2000.0, 0.05), (8000, 0.0), default=0
        # ),
        # const_attention_rate: FloatLike = ScheduledFloat(
        #     (0.0, 0.25), (2000.0, 0.025), default=0
        # ),
        # ff2_skip_rate: FloatLike = ScheduledFloat(
        #     (0.0, 0.1), (2000.0, 0.01), (25000.0, 0.0)
        # ),
        # ff3_skip_rate: FloatLike = ScheduledFloat(
        #     (0.0, 0.1), (2000.0, 0.01), (25000.0, 0.0)
        # ),
        # bypass_skip_rate: FloatLike = ScheduledFloat(
        #     (0.0, 0.5), (2000.0, 0.02), default=0
        ),
    ) -> None:
...
def _test_zipformer_main(causal: bool = True):
    batch_size = 5
    seq_len = 20
